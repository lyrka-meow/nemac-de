...
LauncherModel::LauncherModel(QObject *parent)
    ...
    , m_settings("nemacde", "launcher-applist", this)
    , m_mode(NormalMode)
    , m_firstLoad(false)
{
    // Init datas.
...
void LauncherModel::sendToDock(const QString &key)
{
    ...
    if (index != -1) {
        QDBusMessage message = QDBusMessage::createMethodCall("com.nemac.Dock",
                                                              "/Dock",
                                                              "com.nemac.Dock",
                                                               "add");
...
bool LauncherModel::launch(const QString &path)
{
    ...
    if (index != -1) {
        ...
        // nemac-screenshot needs to be processed.
        if (cmd == "nemac-screenshot") {
