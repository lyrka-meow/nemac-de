...
void Application::initEnvironments()
{
    ...
    // Environment
    qputenv("DESKTOP_SESSION", "Nemac");
    qputenv("XDG_CURRENT_DESKTOP", "Nemac");
    qputenv("XDG_SESSION_DESKTOP", "Nemac");

    // Qt
    qputenv("QT_QPA_PLATFORMTHEME", "nemac");
    qputenv("QT_PLATFORM_PLUGIN", "nemac");
...
{
    QSettings settings(QSettings::UserScope, "nemacde", "language");
...
{
    QSettings settings(QSettings::UserScope, "nemacde", "theme");
...
void Application::initXResource()
{
    ...
    // For nemac-wine
    qputenv("NEMAC_FONT_DPI", QByteArray::number(fontDpi));
...
void Application::initKWinConfig()
{
    ...
    settings.setValue("library", "org.nemac.decoration");
...
// But it won't work if nemacDE is not started by systemd.
...
{
    // bool isNemacDE = QFile::exists("/etc/nemacde");
