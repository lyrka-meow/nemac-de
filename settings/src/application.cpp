    ...
    : QApplication(argc, argv)
{
    ...
    setOrganizationName("nemacde");

    QCommandLineParser parser;
    parser.setApplicationDescription(QStringLiteral("Nemac Settings"));
    ...
    if (!QDBusConnection::sessionBus().registerService("com.nemac.SettingsUI")) {
        QDBusInterface iface("com.nemac.SettingsUI", "/SettingsUI", "com.nemac.SettingsUI", QDBusConnection::sessionBus());
        if (iface.isValid())
    ...
    // Translations
    QLocale locale;
    QString qmFilePath = QString("%1/%2.qm").arg("/usr/share/nemac-settings/translations/").arg(locale.name());
