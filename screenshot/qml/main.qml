...
Item {
    ...
    function refreshImage() {
        image.source = ""
        image.source = "file:///tmp/nemac-screenshot.png"

        selectImage.source = ""
        selectImage.source = "file:///tmp/nemac-screenshot.png"
    }
    ...
    Rectangle {
        ...
        width: sizeLabel.implicitWidth + NemacUI.Units.largeSpacing
        height: sizeLabel.implicitHeight + NemacUI.Units.largeSpacing

        z: 999
        x: selectLayer.x
        y: {
            var newY = selectLayer.y - sizeToolTip.height - NemacUI.Units.smallSpacing
        ...
        color: Qt.rgba(NemacUI.Theme.backgroundColor.r,
                       NemacUI.Theme.backgroundColor.g,
                       NemacUI.Theme.backgroundColor.b, 0.9)
        border.width: 1
        border.color: Qt.rgba(NemacUI.Theme.textColor.r,
                               NemacUI.Theme.textColor.g,
                               NemacUI.Theme.textColor.b, 0.15)
    ...
    Rectangle {
        ...
        width: toolsLayout.implicitWidth + NemacUI.Units.largeSpacing
        height: 36 + NemacUI.Units.smallSpacing
...
//                newY = control.height - tools.height
            ...
            // 选中区域与工具栏高度大于总高度
            if (selectLayer.y + selectLayer.height + tools.height + NemacUI.Units.smallSpacing >= control.height) {
                newY = selectLayer.y - tools.height - NemacUI.Units.smallSpacing
            } else {
                newY = selectLayer.y + selectLayer.height + NemacUI.Units.smallSpacing
            }
        ...
        RowLayout {
            ...
            anchors.bottomMargin: NemacUI.Units.smallSpacing / 2

            ImageButton {
                iconMargins: NemacUI.Units.largeSpacing
                size: 36
