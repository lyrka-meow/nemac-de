...
{
    QSettings settings("nemacde", qApp->applicationName());
...
void FolderModel::paste()
{
    ...
    if (enable) {
        ...
        if (mimeData->hasFormat("application/x-nemac-cutselection")) {
...
void FolderModel::cut()
{
    ...
    mimeData->setData("application/x-nemac-cutselection", QByteArray("1"));
...
{
    QProcess::startDetached("nemac-filemanager", QStringList() << "-e");
}
...
void FolderModel::setWallpaperSelected()
{
    ...
    QDBusInterface iface("com.nemac.Settings", "/Theme",
                         "com.nemac.Theme",
...
{
    QProcess::startDetached("nemac-settings", QStringList() << "-m" << "background");
}
...
{
    if (!url.isEmpty()) {
        QProcess::startDetached("nemac-filemanager", QStringList() << url);
        return;
    }
    ...
    for (const QModelIndex &index : m_selectionModel->selectedIndexes()) {
        KFileItem item = itemForIndex(index);
        if (item.isDir()) {
            QProcess::startDetached("nemac-filemanager", QStringList() << item.url().toLocalFile());
        }
    }
}
