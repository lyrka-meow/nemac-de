...
Item {
    ...
    Rectangle {
        ...
        height: _iconItem.height + _label.paintedHeight + NemacUI.Units.largeSpacing
        x: (parent.width - width) / 2
        y: _iconItem.y

        // (Deprecated) Rectangle rounded corner.
        //color: selected || hovered ? NemacUI.Theme.highlightColor : "transparent"
        color: "transparent"
    ...
    Item {
        ...
        width: parent.width - NemacUI.Units.largeSpacing * 2
    ...
    Label {
        ...
        width: parent.width - NemacUI.Units.largeSpacing * 2 - NemacUI.Units.smallSpacing
        textFormat: Text.PlainText
        elide: Qt.ElideRight
        wrapMode: Text.Wrap
        text: control.fileName
        color: control.GridView.view.isDesktopView ? "white"
                                                   : selected ? NemacUI.Theme.highlightColor
    ...
    Rectangle {
        z: 1
        x: _label.x + (_label.width - _label.paintedWidth) / 2 - (NemacUI.Units.smallSpacing / 2)
        y: _label.y
        width: _label.paintedWidth + NemacUI.Units.smallSpacing
    ...
    DropShadow {
        ...
        horizontalOffset: 1
        verticalOffset: 1
        radius: Math.round(4 * NemacUI.Units.devicePixelRatio)
        samples: radius * 2 + 1
        spread: 0.35
